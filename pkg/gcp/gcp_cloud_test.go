...
func testPrepareForSubmariner() {
	...
	When("the firewall rule doesn't exist", func() {
		...
		Context("", func() {
			...
			It("should correctly insert it", func() {
				...
				if actualRule != nil {
					Expect(actualRule.Name).To(Equal(ingressRuleName))
					Expect(actualRule.Direction).To(Equal("INGRESS"))
					Expect(actualRule.Allowed).To(HaveLen(2))
					Expect(actualRule.Allowed[0]).To(Equal(&compute.FirewallAllowed{
						IPProtocol: "TCP",
						Ports:      []string{"100"},
					}))
					Expect(actualRule.Allowed[1]).To(Equal(&compute.FirewallAllowed{
						IPProtocol: "UDP",
						Ports:      []string{"200"},
					}))
				}
	...
	When("the firewall rule already exists", func() {
		...
		Context("", func() {
			BeforeEach(func() {
				t.gcpClient.EXPECT().UpdateFirewallRule(projectID, ingressRuleName, gomock.Any()).Return(nil)
			})
